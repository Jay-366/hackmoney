...
export function getSqrtRatioAtTick(tick: number): bigint {
    if (tick < MIN_TICK || tick > MAX_TICK) throw new Error("TICK_BOUNDS");
    ...
    if ((absTick & 0x1000) !== 0) ratio = (ratio * 0xd097f3bdfd2022b8845ad8f792aa5841n) >> 128n;
    ...
    let result = ratio >> 32n;
    if ((ratio & mask32) !== 0n) result += 1n;

    return result;
}

function getLiquidityForAmount0(a: bigint, b: bigint, amount0: bigint): bigint {
    if (a > b) [a, b] = [b, a];
    const intermediate = (a * b) / Q96;
    return (amount0 * intermediate) / (b - a);
}

function getLiquidityForAmount1(a: bigint, b: bigint, amount1: bigint): bigint {
    if (a > b) [a, b] = [b, a];
